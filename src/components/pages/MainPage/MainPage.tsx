...
import {getCategoryAll,  getVendorAll , getSubCategoryAll,  getAllVendorLocation} from "../../../http/filtersApi"
const MainPage = () => {
    const dispatch = useAppDispatch();
    useEffect(()=>{
        getCategoryAll().then(resolve=>{dispatch(addCategory(resolve.data)); console.log(resolve)}).catch(f=> console.log(f))
        getAllVendorLocation().then(resolve =>{dispatch(addVendorLocation(resolve)); console.log(resolve) } ).catch(f=> console.log(f))
